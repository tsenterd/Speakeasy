 ...
 */
  ...
  @Override
    public void run() {
        ...
        microphone.stopRecording();
        String variablenamehere = VoiceRecognition.soundText;
            String resultText = result.getBestFinalResultNoFiller();
            
         
                if(variablenamehere.charAt(variablenamehere.length()-1)==('?'))
                {
                    variablenamehere = variablenamehere.substring(0,variablenamehere.length()-1);

                }
                System.out.println("Supposed to say: "+variablenamehere);
           if (resultText.equals(variablenamehere)) {
